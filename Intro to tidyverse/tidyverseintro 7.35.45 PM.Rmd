...
```
Materials can be found on [github](https://github.com/cecusack/EATLAB-R-learning)
...
If you do not have a package, install them in your console using `>install.packages("nameofpackage")`  
Then come back up to this code chunk and upload them.

```{r message=FALSE, warning=FALSE}
# update.packages("tidyverse") # tidyverse changes all the time. make sure you have the most recent version
library(tidyverse) # for life, for data cleaning. This is a suite that includes packages like dplyr (manipulate data), purrrr (for functions and datasets--sometimes faster/better than apply/sappy), forcats (categorical variables), ggplot2 (that good good vis), lubridate (dates and time variables), etc.
...
library(sjmisc) # to move multiple columns at once. you'll notice places i use dplyr::relocate vs sjmisc::move_columns. However, they essentially do the same thing
```
...
```{r}
# So on line 64, we took a look at our data. What if you just wanted to get an idea of what panas data look like
...
PDB_SR[, grep("^panas", colnames(PDB_SR))] # so lines 98-105 do the same thing, but the first one is much easier to read and more flexible. This is a reason we like tidyverse. Happy to speak further on why you would want to use regular expressions approach instead of tidyverse
```
